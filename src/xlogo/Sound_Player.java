...
public class Sound_Player {
     ...
     * @throws myException If the sequence in't valid
     */
    public void cree_sequence(String li) throws myException{
			...
			if (element.equals(":")){
				...
				try{
					...
					if (!element.equals("")&&isnotes(element)==-1&&!element.equals(":"))
					 throw new myException(cadre,Logo.messages.getString("probleme_sequence"));
				}
			}
		}
	}
			   ...
			   sequencer.start( );
				try{ 
					...
					while (temps>0){ //On attends que la séquence soit jouée.
						Thread.sleep(500);
						temps-=32;
						if (myException.lance&&cadre.error) { //On a appuyé sur le bouton stop
							sequencer.close();
							synthesizer.close();
							break;						
						}
